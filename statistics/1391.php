...
<head>
  ...
  <link rel="stylesheet" href="../../assets/css_old/spinkit.css">
        ...
        <ul class="nav navbar-nav navbar-right">
          ...
          <li class="dropdown">
            ...
            <ul class="dropdown-menu">
              <li class="active"><a href="/">1391</a></li>
...
<div class="col-md-8 col-md-offset-2">
  ...
  <h3 class="text-center">Data from Team 1391</h3>
    ...
    <div class="panel-body">
      <div class="col-md-12">
        <form method="POST">
          ...
          <div class="col-md-12">
            <input class="form-control" type="text" name="teams" required autocomplete="off" placeholder="1391 1640 272">
          </div>
          <div class="col-md-12">
            <br>
  ...
  if(isset($_POST['request'])) {
    ...
    header('Location: /statistics/1391/'.urlencode($teams).'');
  }
  if(isset($_GET['teams'])) {
    ...
    curl_setopt_array($curl, array(
      ...
      CURLOPT_HTTPHEADER => array(
        ...
        "X-TBA-Auth-Key: FP8EuLi1ASK3GKlXqHhUZFunb8CmRJHP2ZRW6AVDyI2J5WyDeiVTw8MBR8SZhutHx"
        )
      ));
    ...
    foreach($teams as $key => $data) {
      if(is_numeric($data)) {
        $ghostcheck = $db_connect -> prepare("SELECT COUNT(*) FROM `matches_1391` WHERE `Team_Number` = :requested");
        ...
        if($matches > 0) {
          ...
          <div class="panel-body">';
            $totalgears = array();
            $totalgear_init = $db_connect -> prepare("SELECT `Total_Gears` FROM `matches_1391` WHERE `Team_Number` = :requested");
            ...
            $climbs = array();
            $climb_init = $db_connect -> prepare("SELECT `Rope_Climb` FROM `matches_1391` WHERE `Team_Number` = :requested");
            ...
            $match_numbers_init = $db_connect -> prepare("SELECT `Match_Number` FROM `matches_1391` WHERE `Team_Number` = :requested");
            ...
            foreach($match_numbers_array as $key => $match_numbers_data) {
              $alliance_kpa_init = $db_connect -> prepare("SELECT `KPA` FROM `matches_1391` WHERE `Match_Number` = :requested LIMIT 1");
            ...
            $g1 = array();
            $g1_init = $db_connect -> prepare("SELECT `Gear_R1_Success` FROM `matches_1391` WHERE `Team_Number` = :requested");
            ...
            $g2 = array();
            $g2_init = $db_connect -> prepare("SELECT `Gear_R2_Success` FROM `matches_1391` WHERE `Team_Number` = :requested");
            ...
            $g3 = array();
            $g3_init = $db_connect -> prepare("SELECT `Gear_R3_Success` FROM `matches_1391` WHERE `Team_Number` = :requested");
            ...
            $ground_init = $db_connect -> prepare("SELECT `Teleop_Gear_Ground_Success` FROM `matches_1391` WHERE `Team_Number` = :requested");
            ...
            $chute_init = $db_connect -> prepare("SELECT `Teleop_Gear_Chute_Success` FROM `matches_1391` WHERE `Team_Number` = :requested");
            ...
            $baseline_init = $db_connect -> prepare("SELECT `Auton_Baseline` FROM `matches_1391` WHERE `Team_Number` = :requested");
            ...
            $autongear_init = $db_connect -> prepare("SELECT `Auton_Gear` FROM `matches_1391` WHERE `Team_Number` = :requested");
            ...
            $autongear_location_init = $db_connect -> prepare("SELECT `Auton_Gear_Location` FROM `matches_1391` WHERE `Team_Number` = :requested");
            ...
            echo '<hr>';
            echo '<h4>Fuel:</h4>';
            $fuel_high_success_init = $db_connect -> prepare("SELECT `Teleop_Fuel_High_Success` FROM `matches_1391` WHERE `Team_Number` = :requested");
            ...
            $fuel_high_attempt_init = $db_connect -> prepare("SELECT `Teleop_Fuel_High_Attempt` FROM `matches_1391` WHERE `Team_Number` = :requested");
            ...
            $fuel_low_success_init = $db_connect -> prepare("SELECT `Teleop_Fuel_Low_Success` FROM `matches_1391` WHERE `Team_Number` = :requested");
            ...
            $fuel_low_attempt_init = $db_connect -> prepare("SELECT `Teleop_Fuel_Low_Attempt` FROM `matches_1391` WHERE `Team_Number` = :requested");
            ...
            foreach($match_numbers_array as $key => $notes_match_number) {
              $notes_init = $db_connect -> prepare("SELECT `Notes` FROM `matches_1391` WHERE `Match_Number` = :requested AND `Team_Number` = :data LIMIT 1");
                  ...
                  <div class="panel-heading" role="tab" id="heading'.$notes_match_number.'" style="color: #333333;background-color: #f5f5f5;border-color: #dddddd;border-bottom: 1px solid transparent;">
                    <h4 class="panel-title">
                      ...
                      if($user -> admin($db_connect) OR $data === $_SESSION['Team_Number']) {
                        $admin_scoutname_init = $db_connect -> prepare("SELECT `Scout_Name` FROM `matches_1391` WHERE `Match_Number` = :requested AND `Team_Number` = :data LIMIT 1");
                        ...
                        $admin_scoutnumber_init = $db_connect -> prepare("SELECT `Scout_Team` FROM `matches_1391` WHERE `Match_Number` = :requested AND `Team_Number` = :data LIMIT 1");
              ...
              echo '</div>';
              echo '<br><p style="font-size: 12px;">*OPR (Offensive Power Rating) provided by The Blue Alliance API';
              echo '</div></div>';
            } else {
